...
@Name("articuloNombreValidator")
@BypassInterceptors
@org.jboss.seam.annotations.faces.Validator
public class ArticuloNameValidator implements javax.faces.validator.Validator,
		Serializable {
	...
	private void verifyUniqueness(String nid) {
		...
		java.util.List<Articulo> result = (java.util.List<Articulo>) entityManager
				.createNamedQuery("Articulo.findByNombre")
				.setParameter("nombre", nid).getResultList();

		if (result != null && !result.isEmpty()) {
			message = Interpolator.instance().interpolate(
					"El nombre ya existe", new Object[0]);
